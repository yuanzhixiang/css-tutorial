...
checkbox.addEventListener("click", (etv) => {
  if (checkbox.checked) {
    circle.style.setProperty(`--position-x`, `50px`);
    circle.style.setProperty(`--position-y`, `50px`);
    ...
    circle.style.setProperty(`--position-x`, `0px`);
    circle.style.setProperty(`--position-y`, `0px`);
...
ranges.forEach((range) => {
  range.addEventListener("change", (etv) => {
    circle.style.setProperty(`--${etv.target.id}`, `${etv.target.value}px`);
